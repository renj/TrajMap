...
def get_matched_trip(data, matched_folder):
    ...
    for tid in data.tid.unique():
        global matched_trip
        matched_trip = pd.read_csv(matched_folder+'/matched_trip'+str(tid)+'.csv', sep=' ', names=['x','y','t','x0','y0','x1','y1'])
...
def evaluate(x0, y0, x1, y1):
    ...
    mDist = sorted(mDist)
    print sum(mDist)/len(mDist), mDist[int(len(mDist)*0.5)], mDist[int(len(mDist)*0.67)], mDist[int(len(mDist)*0.8)], mDist[int(len(mDist)*0.9)]
