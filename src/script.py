...
import sacred_trajmap as st
import sacred_trajmap_old as sto
import pp
...
def exam(ex_name, data, side, k, width, max_value, topic_model, maxiter, percent):
    cmd = 'python sacred_trajmap.py with ex_name=%s data=%s side=%d k=%d ratio=None width=%d alpha=0.9 max_value=%f topic_model=%s sel_cand_method=None cands_num=0 maxiter=%d percent=%f' % (ex_name, data, side, k, width, max_value, topic_model, maxiter, percent)
    ret = os.system(cmd)
    if ret == 256:
        print 'KeyboardInterrupt'
        exit()

def cmdline(ex_name, data, topic_model, side, k, width, max_value, maxiter, percent, alpha=0.9, beta=0.01, inc=0, true_pass=False, cut_length=0):
    cmd = 'python sacred_trajmap.py with ex_name=%s data=%s side=%d k=%d ratio=None width=%d alpha=%f beta=%f max_value=%f topic_model=%s sel_cand_method=None cands_num=0 maxiter=%d percent=%f inc=%d true_pass=%s cut_length=%d' % (ex_name, data, side, k, width, alpha, beta, max_value, topic_model, maxiter, percent, inc, str(true_pass), cut_length)
    return cmd
